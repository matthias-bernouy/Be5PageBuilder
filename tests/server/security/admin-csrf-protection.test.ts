...
// /page-builder/api/* are CSRF-vulnerable unless the guard checks Origin /
...
describe("admin CSRF protection", () => {
    test("mutating request without matching Origin must be rejected", async () => {
        const system: any = {
            config: { adminPathPrefix: "/page-builder" },
            auth: {
        ...
        const req = new Request("http://site.com/page-builder/api/page", {
            method: "POST",
